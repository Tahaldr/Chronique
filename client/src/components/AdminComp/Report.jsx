...
const Report = () => {
  ...
  const { reportSelected, setReportSelected } = useContext(AdminDashboardContext);
    ...
    <motion.div
      ...
      <div ref={reportRef} className='h-2/4 w-3/4 md:w-2/4 bg-lighter p-5'>
        <div className='w-full h-full relative text-darker'>
          {/* Report close */}
          <div className='absolute top-0 right-0 cursor-pointer hover:text-light'>
            <RxCross2 className='text-2xl' onClick={() => setReportSelected(null)} />
          </div>

          {/* Download report as PDF */}
          <div className='absolute bottom-0 right-0 cursor-pointer hover:text-light'>
            <PDFDownloadLink
              document={<ReportTemp reportSelected={reportSelected} />}
              fileName={`report-${reportSelected?._id}.pdf`}>
              {({ loading }) => (loading ? '' : <RiDownloadLine className='text-2xl' />)}
            </PDFDownloadLink>
          </div>
          ...
          <div className='flex flex-col items-start gap-5 w-[90%] h-full overflow-y-scroll pr-5'>
            ...
            <div className='flex flex-col gap-1 border-l border-light pl-3 font-smallMedium text-sm'>
              <p>
                <span className='font-smallBold'>Reporter id :</span>{' '}
                {reportSelected?.reporter?._id}
              </p>
              <p>
                <span className='font-smallBold'>Reporter name :</span>{' '}
            ...
            <div className='flex flex-col gap-1 border-l border-light pl-3 font-smallMedium text-sm'>
              <p>
                <span className='font-smallBold'>Reason :</span> {reportSelected?.reason}
              </p>
              <p>
                <span className='font-smallBold'>Content :</span> {reportSelected?.description}
              </p>
            </div>
          </div>
        </div>
