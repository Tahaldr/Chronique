...
const ContextMenu = ({ user }) => {
  ...
  const handleCopyUser = (user) => {
    ...
    navigator.clipboard
      .writeText(userInfo)
      .then(() => showToast({ message: 'User copied to clipboard!', type: 'success' }))
      ...
      {data.map((item) => (
        <div
          ...
          <Tooltip text={item.text}>
            <span className='text-dark hover:text-darkest transition-colors duration-300 ease-in-out text-sm sm:text-base'>
              {item.icon}
            </span>
          </Tooltip>
        </div>
