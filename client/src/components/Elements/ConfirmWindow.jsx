...
const ConfirmWindow = ({ confirming, handleFunc, type }) => {
    ...
    const handleClickOutside = (event) => {
      if (confirmBoxRef.current && !confirmBoxRef.current.contains(event.target)) {
        if (type === 'post') {
          ...
          confirming({ userId: null, confirming: false });
        } else {
  ...
  return (
    <motion.div
      className='w-screen h-screen fixed z-[60] top-0 left-0 flex items-center justify-center bg-black bg-opacity-70'
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      ...
      <div
        ...
        <div className='flex flex-col items-center justify-center gap-1'>
          <h5 className='text-xl text-darker font-mediumPrimary'>Are you sure?</h5>
