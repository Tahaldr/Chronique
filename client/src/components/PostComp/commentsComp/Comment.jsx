      ...
      <div className='flex items-center justify-between'>
        ...
        <Link
          ...
          <div className='flex flex-col gap-0'>
            <p className='font-mediumPrimary text-darker hover:underline'>{comment.author.name}</p>
            <p className='font-small text-sm text-dark'>{moment(comment.createdAt).fromNow()}</p>
          </div>
        </Link>
        {/* Options */}
            ...
            <Tooltip text='options'>
              <SlOptions
                ...
                onClick={(e) => {
                  ...
                  setCommentOptionsPosition(top < window.innerHeight / 2 ? 'up' : 'down');

                  setCommentOptionsShow((prev) => (prev === comment._id ? null : comment._id));
