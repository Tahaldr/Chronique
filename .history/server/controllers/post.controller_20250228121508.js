...
export const getRelatedAuthorPosts = async (req, res) => {
  try {
    ...
    const ensurePostPic = async (posts, requiredCount) => {
      let finalPosts = [];
      let index = 0;

      while (finalPosts.length < requiredCount && index < posts.length) {
        if (posts[index].postPic) {
          finalPosts.push(posts[index]);
        }
        index++;
      }

      while (finalPosts.length < requiredCount) {
    ...
    relatedPosts = await ensurePostPic(relatedPosts, limit);
    ...
    if (relatedPosts.length < limit) {
      ...
      let moreFromAuthor = await Post.find({
        ...
        .sort({ votes: -1 })
        .limit(remaining * 2);
      moreFromAuthor = await ensurePostPic(moreFromAuthor, remaining);
    ...
    if (relatedPosts.length < limit) {
      ...
      let topVotedOthers = await Post.find({
        ...
        .sort({ votes: -1 })
        .limit(remaining * 2);
      topVotedOthers = await ensurePostPic(topVotedOthers, remaining);
